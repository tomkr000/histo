...
for iii in trange(len(files)):
    ...
    if pkl_file not in pkl_files:
        ...
        cp_masks = np.zeros((np.shape(img)[0], np.shape(img)[1]))
        ...
        for i in range(xs):
            for j in range(ys):
                ...
                if i < xs - 1 and j < ys - 1:
                    tile = img[i * width:(i + 1) * width, j * height:(j + 1) * height]
                elif i < xs - 1 and j == ys - 1:
                    tile = img[i * width:(i + 1) * width, j * height:]
                elif i == xs - 1 and j < ys - 1:
                    tile = img[i * width:, j * height:(j + 1) * height]
                else:
                    tile = img[i * width:, j * height:]
